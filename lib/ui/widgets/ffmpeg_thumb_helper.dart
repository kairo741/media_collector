...
class FFmpegThumbHelper {
  ...
  static Future<String?> getThumb(String videoPath) async {
    try {
      final file = File(videoPath);
      if (!await file.exists()) return null;
      final tempDir = Directory.systemTemp.createTempSync('media_collector_thumb');
      final thumbPath = p.join(tempDir.path, '${p.basenameWithoutExtension(videoPath)}_thumb.png');
      ...
      final result = await Process.run(
        ...
        [
          ...
          '-i', videoPath,
          '-frames:v', '1',
          '-vf', 'scale=180:-1:flags=lanczos', // Aumenta resolução e usa melhor algoritmo de scaling
